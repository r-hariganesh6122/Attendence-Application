...
function statusMatchesSessionFilter(status, sessionFilter) {
  if (sessionFilter === "all") {
    return status !== "present"; // Count ANY absence (morningAbsent, afternoonAbsent, or bothAbsent)
  }
  if (sessionFilter === "morning") {
    // Absent from morning if morningAbsent OR bothAbsent (both includes morning)
    return status === "morningAbsent" || status === "bothAbsent";
  }
  if (sessionFilter === "afternoon") {
    // Absent from afternoon if afternoonAbsent OR bothAbsent (both includes afternoon)
    return status === "afternoonAbsent" || status === "bothAbsent";
  }
  if (sessionFilter === "both") {
    // Absent for BOTH sessions (full day) - only bothAbsent status
    return status === "bothAbsent";
  }
  return false;
}
...
function statusIsPresent(status, sessionFilter) {
  if (sessionFilter === "all") {
    return status === "present"; // Only "present" status means present (no absence at all)
  }
  if (sessionFilter === "morning") {
  ...
  if (sessionFilter === "both") {
    // Present for both sessions means NOT absent for both (i.e., not bothAbsent)
    return status !== "bothAbsent";
  }
  return false;
}
...
export default function AcademicCoordinatorDashboard({ user }) {
  ...
  // Handler to fetch teacher password
  const handleGetPassword = async () => {
    if (!changePasswordMobile.trim()) {
    ...
    try {
      const response = await apiCall(
        `/api/teachers?mobile=${encodeURIComponent(changePasswordMobile)}`,
        {
          method: "GET",
        },
      );
      ...
      if (!response.ok || !data.success) {
        alert("Failed to fetch password: " + (data.message || "Unknown error"));
        return;
      }

      setChangePasswordOld(data.password);
    } catch (error) {
                    ...
                    <div className="password-input-wrapper">
                      ...
                      <button
                        ...
                        style={{
                          ...
                          border: "none",
                          borderRadius: "4px",
                          cursor: fetchingPassword ? "not-allowed" : "pointer",
